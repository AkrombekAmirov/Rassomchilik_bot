...
async def get_name(message: types.Message, state: FSMContext):
    ...
    await message.answer("Siz bilan bog'lanish uchun telefon raqamingizni kiriting.")
    await Person.next()
...
async def get_contact(message: types.Message, state: FSMContext):
    ...
    await message.answer("Tanlovda nechta rasm bilan ishtirok etasiz?")
    await Person.next()
...
async def get_school_number(message: types.Message, state: FSMContext):
    ...
    await message.answer("Ta'lim olayotgan maktabingiz raqamini kiriting.")
    await Person.next()
...
async def get_school_number(message: types.Message, state: FSMContext):
    ...
    await message.answer("Necha yoshdasiz?")
    await Person.next()
...
async def get_number(message: types.Message, state: FSMContext):
    ...
    da = (f"Familiya, Ism va sharifingiz: {data.get('name')}\n"
          f"Telefon raqamingiz: {data.get('set_number')}",
          f"Rasmlar soni: {data.get('image_number')}\n",
          f"Ta'lim olayotgan maktabingiz raqami: {data.get('school_number')}\n",
...
async def get_response(call: types.CallbackQuery, state: FSMContext):
    ...
    if data.get("status") == "1":
        await call.message.answer("Ma'lumotingiz qabul qilindi. “Rangtasvir asarlari” nomli bolalar rasmlar tanlovida ishtirok etayotganingizdan minnatdormiz!")
